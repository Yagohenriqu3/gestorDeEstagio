    ...
    <div className='space-y-6 md:space-y-8'>
      <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
        <h2 className='text-2xl md:text-3xl font-bold text-gray-900 flex items-center gap-2'>
          <FiFileText size={28} className='md:w-8 md:h-8' /> Meus Documentos
        </h2>
        <div className='flex flex-wrap gap-2'>
          <span className='px-3 md:px-4 py-2 bg-[#10E686]/20 text-[#10E686] rounded-lg text-xs md:text-sm font-semibold'>
            {documentosEnviados.filter(d => d.status === 'Aprovado').length} Aprovados
          </span>
          <span className='px-3 md:px-4 py-2 bg-red-100 text-red-700 rounded-lg text-xs md:text-sm font-semibold'>
      ...
      {documentosPendentes.length > 0 && (
        <div className='bg-white rounded-2xl shadow-md p-4 md:p-6'>
          <div className='flex flex-wrap items-center gap-2 mb-4 md:mb-6'>
            <FiAlertCircle size={20} className='text-red-500 md:w-6 md:h-6' />
            <h3 className='text-lg md:text-xl font-bold text-gray-900'>Documentos Pendentes</h3>
            <span className='px-2 md:px-3 py-1 bg-red-100 text-red-700 rounded-lg text-xs font-semibold'>
          ...
          <div className='space-y-3 md:space-y-4'>
            {documentosPendentes.map((doc) => {
              ...
              return (
                <div
                  key={doc.id}
                  className={`p-4 md:p-5 border-2 rounded-xl transition-all duration-300 ${
                ...
                >
                  <div className='flex flex-col gap-4'>
                    ...
                    <button className='w-full sm:w-auto bg-gradient-to-r from-[#237EE6] to-[#60C9E6] text-white font-semibold px-4 md:px-6 py-2.5 md:py-3 rounded-lg hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 whitespace-nowrap text-sm md:text-base'>
                      <FiUpload size={18} />
                      Enviar Documento
                    </button>
                  </div>
      ...
      <div className='bg-white rounded-2xl shadow-md p-4 md:p-6'>
        <div className='flex flex-wrap items-center gap-2 mb-4 md:mb-6'>
          <FiCheckCircle size={20} className='text-[#10E686] md:w-6 md:h-6' />
          <h3 className='text-lg md:text-xl font-bold text-gray-900'>Documentos Enviados</h3>
        ...
        <div className='flex flex-wrap gap-2 mb-4 md:mb-6'>
          <button className='px-3 md:px-4 py-2 bg-[#237EE6] text-white font-semibold rounded-lg text-xs md:text-sm'>
            Todos ({documentosEnviados.length})
          </button>
          <button className='px-3 md:px-4 py-2 bg-gray-100 text-gray-700 font-semibold rounded-lg text-xs md:text-sm hover:bg-gray-200 transition-colors'>
        ...
        <div className='space-y-3'>
          {documentosEnviados.map((doc) => (
            <div
              key={doc.id}
              className='flex flex-col md:flex-row md:items-center justify-between p-3 md:p-4 border-2 border-gray-200 rounded-xl hover:border-[#237EE6] transition-all duration-300'
            >
